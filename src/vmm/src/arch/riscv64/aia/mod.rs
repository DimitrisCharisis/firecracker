...
impl AIADevice {
    pub const VERSION: u32 = kvm_bindings::kvm_device_type_KVM_DEV_TYPE_RISCV_AIA;
    ...
    pub fn imsic_properties(&self) -> [u32; 4] {
        [
            ...
            0,
            ::kvm_bindings::KVM_DEV_RISCV_IMSIC_SIZE * self.vcpu_count as u32,
        ]
    }
    ...
    /// Returns the file descriptor of the AIA device
    ...
    pub fn create_device(fd: DeviceFd, vcpu_count: u64) -> Self {
        Self {
            fd,
            properties: [
                AIADevice::get_aplic_addr(),
                AIADevice::get_imsic_addr(),
            ],
            vcpu_count,
        }
    }
    ...
    pub fn init_device_attributes(aia_device: &Self) -> Result<(), AiaError> {
        // Set attributes
        ...
        let aia_imsic_addr = |hart| -> u64 {
            AIADevice::get_imsic_addr() + u64::from(hart) * super::layout::IMSIC_SZ_PH
        };
